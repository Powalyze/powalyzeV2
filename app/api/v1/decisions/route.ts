 ...
 * Retourne toutes les décisions de l'organisation (pour Power BI)
 */
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: decisions, error: queryError } = await supabaseAdmin
      .from('decisions')
      .select(`
        id,
        project_id,
    ...
    if (queryError) {
      console.error('Erreur récupération décisions:', queryError);
      await logApiCall({
        organizationId,
        apiKeyId,
        endpoint: '/api/v1/decisions',
    ...
    console.error('Erreur API /v1/decisions:', error);
